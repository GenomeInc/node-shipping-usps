...
function USPS(args) {
  ...
  function buildIntlRatesRequest(data, options, resource, callback) {
    var root = builder.create('IntlRateV2Request', {headless: true});
    root.att('USERID', $scope.options.username);
    var params = {
      Revision: data.Revision || '2',
      '#list': []
  ...
  function handleIntlRatesResponse(res, callback) {
    return callback(null, res);
  }
  ...
  function buildAddressVerifyRequest(data, options, resource, callback) {
    var root = builder.create('AddressValidateRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    var params = {
      IncludeOptionalElements: data.IncludeOptionalElements || false,
      ReturnCarrierRoute: data.ReturnCarrierRoute || false,
      '#list': []
    };
    for (var i = 0; i < data.addresses.length; i++) {
      data.addresses[i]['@ID'] = i;
      params['#list'].push({Address: data.addresses[i]});
    }
    root.ele(params);
    return root.end({pretty: $scope.options.pretty});
  }

  function handleAddressVerifyResponse(res, callback) {
    return callback(null, res);
  }

  function buildZipCodeLookupRequest(data, options, resource, callback) {
    var root = builder.create('ZipCodeLookupRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    var params = {
      '#list': []
    };
  ...
  function handleZipCodeLookupResponse(res, callback) {
    return callback(null, res);
  }

  function buildCityStateLookupRequest(data, options, resource, callback) {
    var root = builder.create('CityStateLookupRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    var params = {
      '#list': []
    };
    for (var i = 0; i < data.zipCodes.length; i++) {
      data.zipCodes[i]['@ID'] = i;
      params['#list'].push({ZipCode: data.zipCodes[i]});
    }
    root.ele(params);
    return root.end({pretty: $scope.options.pretty});
  }

  function handleCityStateLookupResponse(res, callback) {
    return callback(null, res);
  }


  function buildPriorityMailRequest(data, options, resource, callback) {
    var root = builder.create('PriorityMailRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    root.ele(data);
    return root.end({pretty: $scope.options.pretty});
  }

  function handlePriorityMailResponse(res, callback) {
    return callback(null, res);
  }

  function buildFirstClassMailRequest(data, options, resource, callback) {
    var root = builder.create('FirstClassMailRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    root.ele(data);
    return root.end({pretty: $scope.options.pretty});
  }

  function handleFirstClassMailResponse(res, callback) {
      return callback(null, res);
  }


  function buildStandardBRequest(data, options, resource, callback) {
    var root = builder.create('StandardBRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    root.ele(data);
    return root.end({pretty: $scope.options.pretty});
  }

  function handleStandardBResponse(res, callback) {
    return callback(null, res);
  }

  function buildExpressMailRequest(data, options, resource, callback) {
    var root = builder.create('ExpressMailCommitmentRequest', {headless: true});
    root.att('USERID', $scope.options.username);
    root.ele(data);
    return root.end({pretty: $scope.options.pretty});
  }

  function handleExpressMailResponse(res, callback) {
    return callback(null, res);
  }

  var resources = {
    rates: {f: buildRatesRequest, r: handleRatesResponse, api: 'RateV4', secure: false},
    intlRates: {f: buildIntlRatesRequest, r: handleIntlRatesResponse, api: 'IntlRateV2', secure: false},
    ...
    verify: {f: buildAddressVerifyRequest, r: handleAddressVerifyResponse, api: 'Verify', secure: false},
    zipCodeLookup: {f: buildZipCodeLookupRequest, r: handleZipCodeLookupResponse, api: 'ZipCodeLookup', secure: false},
    cityStateLookup: {f: buildCityStateLookupRequest, r: handleCityStateLookupResponse, api: 'CityStateLookup', secure: false},
    priorityMail: {f: buildPriorityMailRequest, r: handlePriorityMailResponse, api: 'PriorityMail', secure: false},
    firstClassMail: {f: buildFirstClassMailRequest, r: handleFirstClassMailResponse, api: 'FirstClassMail', secure: false},
    standardB: {f: buildStandardBRequest, r: handleStandardBResponse, api: 'StandardB', secure: false},
    expressMail: {f: buildExpressMailRequest, r: handleExpressMailResponse, api: 'ExpressMailCommitment', secure: false}
  };
